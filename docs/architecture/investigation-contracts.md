...
`InvestigationProviderSettings` activates this adapter from `SENTINEL_INVESTIGATION_*` environment variables. With no endpoint, the API remains on the deterministic path.
